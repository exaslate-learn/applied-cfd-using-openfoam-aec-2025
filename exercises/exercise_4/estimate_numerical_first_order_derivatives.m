...
% gradient
yp = 3*x.^2; % Analytical expression
...
for i = 2 : length(y)-1
  ...
  %yp_centralDifference(i, 1) = (y(i+1, 1) - y(i-1, 1)) / (x(i+1, 1) - x(i-1, 1));
...
%plot(x(2:n-1), yp_centralDifference(2:n-1), '--ov', 'linewidth', 2);
...
hold on;
xlabel('x');
%ylabel('d^2\rho/dx^2=2x');
ylabel('d\rho/dx=3*x.^2);
box on;
set(gca, "linewidth", 2, "fontsize", 14)
hold off;
